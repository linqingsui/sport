...
- (SVHTTPRequest*)queueRequest:(NSString*)path
    ...
    NSMutableString *urlString = [[NSMutableString alloc]initWithString:[NSString stringWithFormat:@"%@%@", self.basePath,path]];
    for (int i=0; i<[mergedParameters allKeys].count; i++) {
        if (i==0) {
        ...
        [urlString appendString:[mergedParameters allKeys][i]];
        [urlString appendString:@"="];
        [urlString appendString:[NSString stringWithFormat:@"%@",[mergedParameters valueForKey:[mergedParameters allKeys][i]]]];
    }
    if (!self.addressFields) {
        self.addressFields=[[NSMutableArray alloc]init];
    }
    for (int i=0; i<self.addressFields.count; i++) {
        if ([self.addressFields[i] isEqualToString:urlString]) {
            return nil;
        }
    }
    [self.addressFields addObject:urlString];
